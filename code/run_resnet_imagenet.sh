...
CUDA_VISIBLE_DEVICES=$CUDA_DEVICES python -u main.py simulation -v -ld "log/${MODEL}/${EXP_ID}/pretrain_basetrain" \
 ...
 -p random_seed $RANDOM_SEED -p learning_rate 0.1 -p batch_size 128 -p optimizer SGD \
 ...
 -p scheduler_type Step -p scheduler_warmup_step 3000 -p scheduler_steps "[200,300,350,400,450]" -p scheduler_gamma 0.25 -p scheduler_start_lr_scaler 0.25
...
CUDA_VISIBLE_DEVICES=$CUDA_DEVICES python -u main.py simulation -v -ld "log/${MODEL}/${EXP_ID}/meta_basetrain" \
 ...
 -p random_seed $RANDOM_SEED -p batch_size_training 8 -p batch_size_inference 128 \
 ...
 -p learning_rate 0.005 -p SGDweight_decay 0.000001 -p dropout true -p dropout_rate 0 -p metatrain_frozen False \
 -p metatrain_loss MultiMarginLoss -p loss_param 0.1 -p save_confusion $SAVE_CONFUSION \
 -p pretrainFC linear -p scheduler_type CosineAnnealing -p scheduler_warmup_step 100 -p scheduler_min_lr_scaler 0.01 -p normalize_weightings False
